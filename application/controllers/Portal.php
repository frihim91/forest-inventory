  ...
  public function searchRawEquationAll()
  {
    ...
    if(!empty($_GET)){
      ...
      if($searchFieldArray['keyword']!='')
      {
       ...
       //$validSearchKey='';
       // $validSearchKey='';
      if(isset($searchFieldArray['from']))
      {
        $hmFrom=$searchFieldArray['from'];
      }
      if(isset($searchFieldArray['to']))
      {
        $hmTo=$searchFieldArray['to'];
      }
      if(isset($searchFieldArray['to2']))
      {
        $volumeTo=$searchFieldArray['to2'];
      }
       if(isset($searchFieldArray['from2']))
      {
        $volumeFrom=$searchFieldArray['from2'];
      }


      else if($hmFrom=='' AND $hmTo=='' AND $volumeFrom=='' AND $volumeTo=='')
      {
        foreach($searchFieldArray as $key=>$value)
        {
          if($value!='')
...
if($string==1)
{
  $allometricEquationViewJson=$this->db->query("SELECT a.*,b.*,d.*,dis.*,lg.*,l.*,GROUP_CONCAT(l.location_name) location_name,sr.*,s.ID_Species,s.Species,s.ID_Genus,s.ID_Family,ref.*,f.*,g.*,eco.*,zon.* from ae a
       LEFT JOIN species_group sr ON a.Species=sr.Speciesgroup_ID
       LEFT JOIN species s ON sr.ID_Species=s.ID_Species
       LEFT JOIN family f ON s.ID_Family=f.ID_Family
       LEFT JOIN genus g ON s.ID_Genus=g.ID_Genus
...
else
{
  ...
  $allometricEquationViewJson=$this->db->query("SELECT a.*,b.*,d.*,dis.*,lg.*,l.*,GROUP_CONCAT(l.location_name) location_name,s.ID_Species,sr.*,s.Species,s.ID_Genus,s.ID_Family,ref.*,f.*,g.*,eco.*,zon.* from ae a
       LEFT JOIN species_group sr ON a.Species=sr.Speciesgroup_ID
       LEFT JOIN species s ON sr.ID_Species=s.ID_Species
       LEFT JOIN family f ON s.ID_Family=f.ID_Family
       LEFT JOIN genus g ON s.ID_Genus=g.ID_Genus
...
if($string==1)
{
  $allometricEquationViewcsv=$this->db->query("SELECT a.*,b.*,d.*,dis.*,lg.*,l.*,ci.*,un.*,u.*,GROUP_CONCAT(l.location_name) location_name,sr.*,s.ID_Species,s.Species,s.ID_Genus,s.ID_Family,ref.*,f.*,g.*,eco.*,zon.* from ae a
       LEFT JOIN species_group sr ON a.Species=sr.Speciesgroup_ID
       LEFT JOIN species s ON sr.ID_Species=s.ID_Species
       LEFT JOIN family f ON s.ID_Family=f.ID_Family
       LEFT JOIN genus g ON s.ID_Genus=g.ID_Genus
...
,'SEY','SEE','AIC','FI','Bias_correction',' Ratio_equation','Segmented_equation','Contributor','Contributor_name','Operator_name','email_contact','Remark','Verified'));
                  ...
                  foreach ($allometricEquationViewcsv as $data) {
                      fputcsv($handle, array($data["ID_AE"], $data["ID_RD"], $data["Population"], $data["Tree_type"], $data["Vegetation_type"], $data["location_group"],$data["X"], $data["ID"], $data["group_id"], $data["location_id"],$data["Unit_X"], $data["Z"], $data["Unit_Z"], $data["W"], $data["Unit_W"], $data["U"],
                        ...
                        ,$data["Reference"],$data["Author"],$data["Year"],$data["Title"],$data["Journal"],$data["Volume"],$data["Issue"],$data["Page"],$data["URL"],$data["PDF_label"],$data["Species"],$data["Family"],$data["Genus"],$data["Division"],$data["District"],$data["THANAME"],$data["UNINAME"],$data["FAOBiomes"],$data["AEZ_NAME"],$data["Zones"],$data["R2"],$data["R2_Adjusted"],$data["Corrected_for_bias"],$data["MSE"],$data["RMSE"],$data["SEY"],$data["SEE"],$data["AIC"] ,$data["FI"],$data["Bias_correction"],$data["Ratio_equation"],$data["Segmented_equation"],$data["Contributor"],$data["Contributor_name"],$data["Operator_name"],$data["email_contact"],$data["Remark"],$data["Verified"]));
                      $i++;
                  }
                      fclose($handle);
                  exit;
...
,'ID_EF_IPCC','Reference','Lower_Confidence_Limit','Upper_Confidence_Limit','Type_of_Parameter','group_location','Contributor ','Contributor_name ','location_name','Division  ',' District  ',' THANAME',' UNINAME',' FAOBiomes',' Reference',' Author',' Year',' Title',' Journal',' Volume',' Issue',' Page',' URL',' PDF_label',' Family',' Genus',' Species',' Zones'));
                  $i = 1;
                  foreach ($biomassExpansionFacView as $data) {
                      fputcsv($handle, array($data["ID_EF"], $data["EmissionFactor"], $data["ID_LandCover"], $data["Species"], $data["Age_yr"], $data["Height_m"], $data["Volume_m3_ha"]
                        ...
                        , $data["group_location"], $data["Contributor"], $data["Contributor_name"], $data["location_name"],$data["Division"], $data["District"], $data["THANAME"], $data["UNINAME"], $data["FAOBiomes"],$data["Reference"],$data["Author"],$data["Year"],$data["Title"],$data["Journal"],$data["Volume"],$data["Issue"],$data["Page"],$data["URL"],$data["PDF_label"], $data["Family"],
                        $data["Genus"],$data["Species"], $data["Zones"]));
                      $i++;
                  }
                      fclose($handle);
                  exit;
...
header('Content-disposition: attachment; filename=EmissionFactor.json');
  ...
  public function biomassExpansionFacSpeciesView($specis_id)
  {
      ...
      $config["total_rows"] = $total_ef;
       ...
       $total_ef=$this->db->query("SELECT e.ID_EF, e.EmissionFactor,e.Unit,e.Value,ref.Author,ref.Reference,ref.Year,d.Division,dis.District,l.location_name,GROUP_CONCAT(lg.location_id),s.Species,g.Genus,f.Family,b.FAOBiomes,eco.AEZ_NAME,zon.Zones from ef e
       LEFT JOIN species s ON e.Species=s.ID_Species
       LEFT JOIN family f ON s.ID_Family=f.ID_Family
       LEFT JOIN genus g ON s.ID_Genus=g.ID_Genus
       LEFT JOIN reference ref ON e.Reference=ref.ID_Reference
...
if($string==1)
{
  $rawDataViewjson=$this->db->query("SELECT r.*,u.THANAME,un.UNINAME,ci.Contributor_name,sl.local_name,l.location_name,GROUP_CONCAT(lg.location_id),b.FAOBiomes,d.Division,dis.District,s.Species,ref.Reference,ref.Year,ref.Author,f.Family,g.Genus,zon.Zones,eco.AEZ_NAME from rd r
       LEFT JOIN species_group sr ON r.Speciesgroup_ID=sr.Speciesgroup_ID
       LEFT JOIN species s ON sr.ID_Species=s.ID_Species
       LEFT JOIN species_localname sl ON s.ID_Species=sl.Species_ID
       LEFT JOIN family f ON s.ID_Family=f.ID_Family
...
if($string==1)
{
  $rawDataViewcsv=$this->db->query("SELECT r.*,u.THANAME,un.UNINAME,ci.Contributor_name,sl.local_name,l.*,l.location_name,GROUP_CONCAT(lg.location_id),b.FAOBiomes,d.Division,dis.District,s.Species,ref.Reference,ref.Year,ref.Author,f.Family,g.Genus,zon.Zones,eco.AEZ_NAME from rd r
       LEFT JOIN species_group sr ON r.Speciesgroup_ID=sr.Speciesgroup_ID
       LEFT JOIN species s ON sr.ID_Species=s.ID_Species
       LEFT JOIN species_localname sl ON s.ID_Species=sl.Species_ID
       LEFT JOIN family f ON s.ID_Family=f.ID_Family
...
header("Expires: 0");
$handle = fopen('php://output', 'w');
fputcsv($handle, array('ID','ID_RD', 'Tree_type','Vegetation_type','Division','District','THANAME','UNINAME','LatDD'
...
,'D_Bark_g','D_Bark_kg',' D_Stem_with_Bark_g','D_Stem_without_Bark_g',' D_Stem_without_Bark_kg',' D_Stump_kg',' D_Buttress_kg','D_Roots_kg',' ABG_g','ABG_kg',' BGB_kg','Reference','Contributor_name','Remark'
));
                  $i = 1;
                  foreach ($rawDataViewcsv as $data) {
                      fputcsv($handle, array($data["ID"], $data["ID_RD"], $data["Tree_type"], $data["Vegetation_type"], $data["Division"], $data["District"]
                        ...
                        ,$data["WD_AVG_gcm3"],$data["D_Bole_kg"],$data["D_Branch_kg"],$data["D_Foliage_g"],$data["D_Foliage_kg"],$data["D_Branch_g"],$data["D_Bark_g"],$data["D_Bark_kg"],$data["D_Stem_with_Bark_g"] ,$data["D_Stem_without_Bark_g"],$data["D_Stem_without_Bark_kg"],$data["D_Stump_kg"],$data["D_Buttress_kg"],$data["D_Roots_kg"],$data["ABG_g"],$data["ABG_kg"],$data["BGB_kg"],$data["Reference"],
                        $data["Contributor_name"],$data["Remark"] ));
                      $i++;
                  }
                      fclose($handle);
                  exit;
...
if($string==1)
{
  $woodDensityViewjson=$this->db->query("SELECT w.*,ref.Author,ref.Reference,ref.Year,sl.local_name,u.THANAME,un.UNINAME,d.Division,dis.District,l.*,GROUP_CONCAT(lg.location_id),s.Species,g.Genus,f.Family,b.FAOBiomes,eco.AEZ_NAME,zon.Zones,ci.Contributor_name from wd w
       LEFT JOIN species s ON w.ID_species=s.ID_Species
       LEFT JOIN species_localname sl ON s.ID_Species=sl.Species_ID
       LEFT JOIN family f ON s.ID_Family=f.ID_Family
       LEFT JOIN genus g ON s.ID_Genus=g.ID_Genus
